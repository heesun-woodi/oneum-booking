...
export interface FreeHoursInfo {
  usedHours: number
  limitHours: number
...
interface Props {
  ...
  /** 세대 무료 사용 현황 (놀터 + 세대원일 때만 조회됨). 조회 중이면 null */
  freeHours: FreeHoursInfo | null
  prepaidPurchases: PrepaidLike[]
  /** 예약하려는 달 (1~12). 무료 한도는 사용일이 속한 달 기준이다. */
  monthLabel: number
}
...
export function BookingChargeSummary({
  ...
  // 무료 사용량을 아직 못 받았으면 계산을 보류한다 (잘못된 금액을 잠깐이라도 보여주지 않는다)
  if (showsNolterQuota && freeHours === null) {
    return (
  ...
  const charge = computeBookingCharge({
    ...
    space,
    requestedHours,
    freeHoursUsedThisMonth: freeHours?.usedHours ?? 0,
    prepaidPurchases,
  })
  ...
  return (
    <div className="space-y-3">
      {/* 세대 무료 한도 현황 */}
      {showsNolterQuota && freeHours && (
        <div
          className={`rounded-lg p-4 border ${
            freeHours.remainingHours > 0
        ...
        >
          <p
            className={`text-sm font-semibold ${
              freeHours.remainingHours > 0 ? 'text-green-800' : 'text-yellow-800'
            }`}
          >
            🎟 {monthLabel}월 세대 무료: {formatHours(freeHours.usedHours)} /{' '}
            {formatHours(freeHours.limitHours)} 사용
          </p>
          <div className="w-full bg-white/70 rounded-full h-2 my-2">
            <div
              className={`h-2 rounded-full transition-all ${
                freeHours.remainingHours > 0 ? 'bg-green-400' : 'bg-yellow-400'
              }`}
              style={{
                width: `${Math.min(
                  100,
                  freeHours.limitHours > 0
                    ? (freeHours.usedHours / freeHours.limitHours) * 100
                    : 0
                )}%`,
              }}
            />
          ...
          <p
            className={`text-xs ${
              freeHours.remainingHours > 0 ? 'text-green-600' : 'text-yellow-700'
            }`}
          >
            {freeHours.remainingHours > 0
              ? `남은 무료 시간: ${formatHours(freeHours.remainingHours)}`
      ...
      {isResident && space === 'soundroom' && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <p className="text-xs text-green-600 mt-1">놀터 월 20시간 무료 한도와 별개로 운영됩니다.</p>
        </div>
      )}
      ...
      {selectedSlotCount > 0 && (
        ...
        >
          ...
          {showsNolterQuota && charge.freeHours > 0 && charge.freeHoursRemainingAfter < 2 && (
